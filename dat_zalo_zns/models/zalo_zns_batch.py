...
class ZaloZnsBatch(models.Model):
    ...
    template_id = fields.Many2one(
        'zalo.zns.template', string='Template', required=True)
    ...
    recipient_ids = fields.Many2many(
        'res.partner', 'batch_id', string='Recipient')
    message_ids = fields.One2many(
        'zalo.zns.message', 'batch_id', string='Messages')
    ...
    def _generate_unique_name(self, date, model_name):
        ...
        day = date.strftime('%d')
        # Use 4 last charactors of sequence for suffix
        suffix = sequence.next_by_code('zalo.zns.batch')[-4:]
    ...
    def action_done(self):
        if any(message.state != 'sent' for message in self.message_ids):
            raise UserError(
                _("Cannot mark as done. There are unsent messages in this batch."))
