...
class DashboardBlock(models.Model):
    ...
    def get_default_action(self):
        ...
        action_id = self.env.ref(
            'dat_dashboard.dashboard_view_action')
        if action_id:
            return action_id.id
        return False
    ...
    model_id = fields.Many2one(
        'ir.model',
        string='Model',
        domain="[('model', 'in', ['ticket.helpdesk', 'step.reception.timeliness.report', 'ticket.step.status',"
               " 'hr.employee', 'sale.order', 'res.partner', 'stock.lot',])]",
    ...
    def get_dashboard_vals(self, action_id, start_date=None, end_date=None):
        ...
        for rec in self.env['dashboard.block'].sudo().search(
            ...
            vals = {'id': rec.id, 'name': rec.name, 'type': rec.type,
                    ...
                    'measured_field': rec.measured_field_id.field_description if rec.measured_field_id else '',
                    ...
                    for record in records:
                        if record.get('name') and type(
                            ...
                            x_axis.append(record.get('name')[self._context.get(
                                'lang') or 'en_US'])
                        elif type(record.get(rec.group_by_id.name)) == dict:
                            x_axis.append(record.get(rec.group_by_id.name)[self._context.get(
                                'lang') or 'en_US'])
                        else:
                            x_value = record.get(rec.group_by_id.name)
                            if isinstance(x_value, int) and (rec.group_by_id.ttype in ('many2one', 'many2many')):
                                x_record = self.env[rec.group_by_id.relation].with_context(lang=self._context.get('lang') or 'en_US').browse(x_value)
                                x_axis.append(x_record.name)
                            else:
