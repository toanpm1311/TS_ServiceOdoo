...
class TicketHelpDesk(models.Model):
    ...
    ticket_rating_rate = fields.Integer(related='ticket_rating.rate')
    ticket_rating_note = fields.Text(related='ticket_rating.note')
    ...
    @api.constrains('product_warranty_status', 'stock_lot_id', 'stock_lot_id.warranty_end_date')
    def _check_product_warranty_status(self):
        for rec in self:
            ...
            if rec.stock_lot_id and rec.stock_lot_id.warranty_end_date:
                end_dt = fields.Datetime.to_datetime(rec.stock_lot_id.warranty_end_date)
            else:
                end_dt = fields.Datetime.to_datetime(date(2000, 1, 1))
    ...
    def _onchange_owner_id(self):
        self.owner_id = self.stock_lot_id.owner_id.id
    ...
    def _compute_product_warranty_status(self):
        for rec in self:
            ...
            if rec.stock_lot_id and rec.stock_lot_id.warranty_end_date and rec.create_date:
                if rec.create_date <= rec.stock_lot_id.warranty_end_date:
    ...
    def _onchange_owner_id(self):
        if self.owner_id:
            ...
            if self.owner_id.sale_person:
                self.saleperson_id = self.owner_id.sale_person
            elif self.customer_id.sale_person:
                self.saleperson_id = self.customer_id.sale_person
            elif self.branch:
                sale_department_ids = [
                ...
                if sale_department and sale_department.manager_id:
                    ...
                    self.saleperson_id = assigned_user.employee_id if assigned_user and assigned_user.employee_id else False
            else:
    ...
    def action_next_step_wf1_step5a_confirm_replace_and_serial(self):
        ...
        if self.is_exchange_1_1:
            ...
            if not has_new_serial:
                raise ValidationError(_("Đổi 1-1: phải nhập/chọn 'Serial mới' trước khi xử lý tiếp."))
    ...
    def action_next_step_wf1_step7_product_delivery(self):
        ...
        if self.is_exchange_1_1:
            ...
            has_new_serial = bool(getattr(self, "new_stock_lot_id", False)) or bool(getattr(self, "replace_serial_number", False))
            if not has_new_serial:
                raise ValidationError(_("Đổi 1-1: thiếu 'Serial mới' / 'Replace Serial Number'."))
    ...
    def action_next_step_wf1_step4_material_dispatch(self):
        ...
        if self.product_warranty_status == 'warranty':
            sale_order = self.env['sale.order'].search([('ticket_id', '=', self.id)], limit=1)
        else:
            sale_order = self.env['sale.order'].search([('ticket_id', '=', self.id), ('status', '=', 'confirmed')],
                                                       limit=1)
        if sale_order:
    ...
    def action_next_step_wf1_step5_repair(self):
        ...
        # Create new SO in SAP system
        if self.product_warranty_status == 'warranty':
            sale_order = self.env['sale.order'].search([('ticket_id', '=', self.id)], limit=1)
        else:
            sale_order = self.env['sale.order'].search([('ticket_id', '=', self.id), ('status', '=', 'confirmed')], limit=1)
    ...
    def _bnk_call_api(self, api_link):
        """
        Gọi POST tới BnK. Nếu response=200 → trả thông báo thành công.
        """
        self.ensure_one()
